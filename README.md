...
### Kvalitetsreflektion kring data och utvärdering

Eftersom datahämtningen har gjorts i ett separat projekt har detta arbete främst
fokuserat på analys och modellering av den färdiga CSV-datan. Vissa geografiska
features, till exempel koordinater och avstånd, bör därför tolkas som ungefärliga
indikatorer snarare än exakta lägesmått.

Modellen utvärderas med en train/test-split med fast random state. Det ger en
reproducerbar och tydlig jämförelse mellan modellerna, men bostadsmarknaden
förändras över tid. En möjlig vidareutveckling vore därför att även testa en
tidsbaserad uppdelning, där modellen tränas på äldre försäljningar och testas på
nyare försäljningar.

Utgångspris används som feature eftersom det ofta är känt innan en bostad säljs
och har stark koppling till slutpriset. Det betyder samtidigt att modellen främst
ska ses som ett stöd när utgångspris finns tillgängligt, inte som en helt
fristående värderingsmodell.
...
Projektet visar att det går att bygga en fungerande regressionsmodell för att
uppskatta bostadspriser i Partille kommun med hjälp av verklig bostadsdata.
Resultaten visar tydliga mönster i datan, särskilt att större datamängd ger en
mer stabil modell i detta fall. 3-årsmodellen presterar bäst av de testade
perioderna och ger den bästa balansen mellan felmått, förklaringsgrad och
datamängd.

Samtidigt visar projektet också att bostadspriser är svåra att prediktera exakt.
Viktiga faktorer som skick, mikroläge, ränteläge och budgivningssituation saknas
eller är svåra att mäta. Därför är modellens viktigaste värde inte att ge ett
perfekt pris, utan att visa hur maskininlärning kan användas för att analysera
bostadsdata, jämföra modeller och skapa ett strukturerat beslutsstöd.
